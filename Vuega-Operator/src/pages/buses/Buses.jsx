    ...
    if (row.status === "pending-approval") {
      return (
        <div className="flex items-center justify-end gap-2">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-yellow-600 bg-v-secondary border border-v-secondary-border">
            <FaClock size={14} />
    ...
    if (row.status === "rejected") {
      return (
        <div className="flex items-center justify-end gap-2" title="This bus request was rejected by the Super Admin">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-v-critical bg-v-critical-light border border-v-critical-border cursor-not-allowed">
            <FaBan size={14} />
            Request Rejected
          </span>
        </div>
    ...
    return (
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-v-text-secondary border border-v-border hover:bg-v-accent/20 hover:border-v-accent-border transition-colors"
        >
          <FaEye size={14} />
          View
        </button>
        <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-v-text-secondary border border-v-border hover:bg-v-secondary hover:border-v-secondary-border transition-colors">
          <FaPencilAlt size={14} />
          Edit
        </button>
      </div>
  ...
  return (
    <div className="max-w-[1360px] mx-auto flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-v-text text-2xl font-bold tracking-tight">
            Buses
          </h2>
          <p className="text-v-text-muted text-sm mt-0.5">
            Manage your fleet — request, monitor, and track bus status.
          </p>
        </div>
        <button
          onClick={() => setShowRequestModal(true)}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold bg-v-accent hover:bg-v-accent-hover text-v-text border border-v-accent-border transition-colors shadow-sm"
        >
          <FaPlus size={18} />
          Request New Bus
        </button>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[220px] max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-v-border bg-v-primary-bg text-sm text-v-text placeholder:text-v-text-placeholder focus:outline-none focus:ring-2 focus:ring-v-accent focus:border-v-accent-border transition-colors"
          />
        </div>
        ...
        <div className="relative">
          ...
          <select
            ...
            className="pl-9 pr-8 py-2.5 rounded-lg border border-v-border bg-v-primary-bg text-sm text-v-text-secondary focus:outline-none focus:ring-2 focus:ring-v-accent focus:border-v-accent-border transition-colors appearance-none cursor-pointer"
          >
      ...
      <div className="bg-v-primary-bg border border-v-border rounded-xl shadow-sm overflow-hidden">
        ...
        {/* Footer */}
        <div className="px-5 py-3 border-t border-v-border bg-v-secondary/20 text-xs text-v-text-muted">
