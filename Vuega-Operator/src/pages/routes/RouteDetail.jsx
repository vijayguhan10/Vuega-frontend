  ...
  if (!route) {
    return (
      <div className="p-6 flex flex-col items-center justify-center h-64 text-center space-y-3">
        <p className="text-v-text-muted text-sm">Route not found.</p>
        <button
          onClick={() => navigate('/routes')}
          className="text-sm text-v-accent-dark hover:underline"
        >
          ← Back to Routes
        </button>
      </div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-v-text">
              {route.fromCity} → {route.toCity}
            </h1>
            <p className="text-sm text-v-text-muted mt-0.5">Route #{route.id}</p>
          </div>
        </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {items.map((item) => (
        <div key={item.label} className="space-y-1">
          <p className="text-[11px] font-semibold text-v-text-muted uppercase tracking-wider">
            {item.label}
          </p>
          <p className="text-sm font-medium text-v-text">{item.value}</p>
        </div>
      ))}
    </div>
  );
