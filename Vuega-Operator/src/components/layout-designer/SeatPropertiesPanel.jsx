...
export default function SeatPropertiesPanel({
 ...
 <p className=" text-v-text-muted">Click a seat to edit · Click a column header to bulk-edit</p>
 </div>
 </aside>
 );
 }
 ...
 <h3 className=" font-bold text-v-text tracking-tight">Column Properties</h3>
 <button
 ...
 <div>
 <p className=" font-semibold text-v-text">Column {colIdx < 2 ? colIdx + 1 : colIdx}</p>
 <p className=" text-v-text-muted">{(columnSeats || []).length} seat{(columnSeats || []).length !== 1 ? 's' : ''} in column</p>
 </div>
 </div>
 ...
 {/* Header */}
 <div className="flex items-center justify-between">
 <h3 className=" font-bold text-v-text tracking-tight">Seat Properties</h3>
 <button
 onClick={clearSelection}
 className="text-v-text-muted hover:text-v-text transition-colors"
 title="Close"
 ...
 <span className=" font-bold text-v-text">{seat.seatNumber}</span>
 </div>
 <div>
 <p className=" font-semibold text-v-text">Seat #{seat.seatNumber}</p>
 <p className=" text-v-text-muted capitalize">{seat.deck} deck · {seat.type}</p>
 </div>
 </div>
 ...
 {seat.deck === 'upper' && (
 <p className=" text-v-text-muted">Upper deck seats are always sleeper</p>
 )}
 ...
 className="w-full py-2 rounded-lg font-medium text-green-700 border border-green-300 hover:bg-green-50 transition-colors"
 >
 Restore Seat
 </button>
 ) : (
...
function Toggle({ label, description, checked, onChange, accentClass = 'peer-checked:bg-v-accent-dark' }) {
 ...
 <div>
 <span className=" font-medium text-v-text-secondary group-hover:text-v-text transition-colors block leading-tight">
 {label}
 </span>
 {description && (
 <span className=" text-v-text-muted leading-tight">{description}</span>
 )}
 </div>
 </label>
 );
