...
const SideBar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        ref={sidebarRef}
        className={`fixed top-0 left-0 z-40 w-64 h-screen bg-white border-r border-slate-200 transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"
          } sm:translate-x-0`}
      >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive
                    ? "bg-highlight text-blue-900 shadow-sm"
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon
                      size={20}
                      className={isActive ? "text-blue-900" : "text-text-muted"}
